...
auto PBKDF2_HMAC_SHA_256( const unsigned char* salt, const char* pass )
{
    ...
    PKCS5_PBKDF2_HMAC( pass, std::strlen( pass ), salt, std::strlen( ( char* ) salt ), iterations, EVP_sha256(), outputBytes, digest );

    for( unsigned int i = 0; i < sizeof( digest ); i++ )
    {
        hstream << std::hex << std::setfill('0') << std::setw(2) << std::right << (255 & digest[ i ]); 
    } 
    return hstream.str();
}
