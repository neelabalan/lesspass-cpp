...
create_salt_profile(argparse::ArgumentParser &);
...
std::pair<std::string, BigInt>
_consume_entropy(std::string, BigInt, std::string, unsigned int);
...
int main(int argc, char *argv[])
{
    ...
    std::tie(password, password_entropy) = _consume_entropy(
        std::string(""), entropy, set_of_characters, (password_length - rules.size())
    );

    BigInt character_entropy = std::string("0");
    std::string chars{};

    std::tie(character_entropy, chars) =
        _get_one_char_per_rule(password_entropy, rules);

    auto final_password =
        _insert_string_pseudo_randomly(character_entropy, password, chars);

    std::cout << final_password;
}

std::string _insert_string_pseudo_randomly(
    BigInt character_entropy, std::string generated_password, std::string chars
)
{
    for (auto &letter : chars)
    {
        BigInt remainder = generated_password.length();
        std::tie(character_entropy, remainder) =
            util::divmod(character_entropy, remainder);
        generated_password.insert(remainder.to_int(), 1, letter);
    }
    return generated_password;
}
std::pair<BigInt, std::string>
_get_one_char_per_rule(BigInt password_entropy, std::vector<SUBSET> rules)
{
    ...
    for (auto &rule : rules)
    {
        std::tie(value, password_entropy) = _consume_entropy(
            std::string(""), password_entropy, CHARACTER_MAP.at(rule), 1
        );
...
std::string _get_set_of_characters(std::vector<SUBSET> rules)
{
    ...
    for (auto &rule : rules)
    {
        if (rule == SUBSET::lowercase)
        {
            set_of_characters += CHARACTER_MAP.at(SUBSET::lowercase);
        }
        if (rule == SUBSET::uppercase)
        {
            set_of_characters += CHARACTER_MAP.at(SUBSET::uppercase);
        }
        if (rule == SUBSET::digits)
        {
            set_of_characters += CHARACTER_MAP.at(SUBSET::digits);
        }
        if (rule == SUBSET::symbols)
        {
            set_of_characters += CHARACTER_MAP.at(SUBSET::symbols);
        }
    }
    return set_of_characters;
}
std::pair<std::string, BigInt> _consume_entropy(
    std::string gen_pass, BigInt quotient, std::string chars, unsigned int len
)
{
    BigInt remainder;
...
{
    return util::hexstream_to_bigint(PBKDF2_HMAC_SHA_256(
        reinterpret_cast<const unsigned char *>(salt.c_str()), pass.c_str()
    ));
}
...
{
    return (
        profile["site"] + profile["login"] + util::int_to_hexstr(profile["counter"])
    );
}

std::unordered_map<std::string, std::string>
create_salt_profile(argparse::ArgumentParser &args)
{
...
std::vector<SUBSET> _get_rules(argparse::ArgumentParser &args)
{
    ...
    if (args.get<bool>("-l"))
    {
        rules.push_back(SUBSET::lowercase);
    }
    if (args.get<bool>("-u"))
    {
        rules.push_back(SUBSET::uppercase);
    }
    if (args.get<bool>("-d"))
    {
        rules.push_back(SUBSET::digits);
    }
    if (args.get<bool>("-s"))
    {
        rules.push_back(SUBSET::symbols);
    }
...
{
    args.add_argument("--site").help("site").default_value(std::string(" ")).required();
